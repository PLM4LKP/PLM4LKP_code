...
class TagRecommandModel(nn.Module):
    def __init__(self, tags_count, freeze_bert_text=False, freeze_bert_code=False, hidden_dropout_prob=0.1):
        ...
        print('加载模型')
        self.tokenizer_text = AutoTokenizer.from_pretrained('lanwuwei/BERTOverflow_stackoverflow_github')

        self.bert_text = AutoModel.from_pretrained('lanwuwei/BERTOverflow_stackoverflow_github')
